...
目前 80-node Core Atlas 的主要分類只覆蓋 20 個 domain registry 中的 8 個領域。每批 100 題的目的，是先量測品質與成本並擴大有效覆蓋，不是反覆堆高少數題族的數量。

- 沒有 Canonical MKO 的領域不得直接從 `atlas_seed` 出題；應先由主架構流程完成研究、物化與審查邊界。
- 在核心領域都至少具有一個合格 MKO 與一個通過機械檢查的題族前，優先選擇尚未覆蓋的領域。
- 同一題族若已有足夠練習密度，下一批應轉向新的概念、方法或表示方式。
- 是否把單批上限提高到 1,000、10,000 或更高，必須依實測 token 成本、每題位元組、重複率、驗證時間與資料庫查詢表現決定。
