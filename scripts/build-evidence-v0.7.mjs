...
async function buildAdapterEvidence({ sourceDir, schemaPath, label, adapt, errors }) {
  ...
  const manifestSchema = JSON.parse(await fs.readFile(path.join(ROOT, schemaPath), 'utf8'))
  ...
  for (const name of names) {
    ...
    if (!validateManifest(manifest)) {
      errors.push(...formatSchemaErrors(`${label} manifest ${name}`, validateManifest.errors))
      continue
    }
    try {
      evidence.push(await adapt(manifest, {
        subject: byId.get(manifest.subject_id),
        root: ROOT
      }))
    } catch (error) {
      errors.push(`${label} manifest ${name}: ${error.message}`)
    }
  }
  return evidence
}
...
const errors = []
const pythonEvidence = await Promise.all(
  Object.entries(pythonConfigs).map(([subjectId, config]) => buildPythonEvidence(subjectId, config))
)
const felraEvidence = await buildAdapterEvidence({
  sourceDir: 'evidence-sources/felra',
  schemaPath: 'schemas/felra-run-manifest.schema.json',
  label: 'FELRA',
  adapt: adaptFelraManifest,
  errors
})
const leanEvidence = await buildAdapterEvidence({
  sourceDir: 'evidence-sources/lean',
  schemaPath: 'schemas/lean-run-manifest.schema.json',
  label: 'Lean',
  adapt: adaptLeanManifest,
  errors
})

const expected = [...pythonEvidence, ...felraEvidence, ...leanEvidence].sort((a, b) => a.id.localeCompare(b.id))
const expectedIndex = {
  schema_version: 'ocme-evidence-index-v0.3',
...
if (mode === 'write') {
  ...
  for (const evidence of expected) {
    ...
    if (!isDeepStrictEqual(actual, evidence)) {
      errors.push(`${evidence.subject_id}/${evidence.producer.id}: Evidence Object drifted; run npm run build:evidence`)
    }
  }
}
...
const result = {
  ok: errors.length === 0,
  schema_version: 'ocme-evidence-build-v0.3',
  mode,
...
await fs.writeFile(path.join(ROOT, 'artifacts', 'evidence-verification-v0.7.json'), `${JSON.stringify(result, null, 2)}\n`, 'utf8')

if (errors.length) {
  console.error(errors.join('\n'))
  process.exit(1)
}
console.log(`Evidence ${mode} passed: ${expected.length} object(s), including ${felraEvidence.length} FELRA and ${leanEvidence.length} Lean object(s).`)
