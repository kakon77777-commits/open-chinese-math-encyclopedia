...
function renderMethods() {
  ...
  $('#view').innerHTML = `<section class="page-shell">
    ...
    <div class="domain-cloud">${state.domains.map((domain, index) => `<article><span>${String(index + 1).padStart(2, '0')}</span><h3>${escapeHtml(domain.title_zh)}</h3><p>${escapeHtml(domain.description_zh)}</p></article>`).join('')}</div>
