...
const subset = await loadObject('mko-subset')
...
assert.equal(validateV04(structuredClone(subset)), true)
...
const malformedSubset = structuredClone(subset)
delete malformedSubset.formula.semantic_ast.superset
assert.equal(validateV04(malformedSubset), false)
