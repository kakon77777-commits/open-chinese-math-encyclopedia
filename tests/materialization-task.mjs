...
assert.equal(new Set(tasks.map(task => task.task_id)).size, 66)
assert.equal(new Set(tasks.map(task => task.atlas_id)).size, 66)
assert.equal(new Set(tasks.map(task => task.target_mko_id)).size, 66)
...
console.log('Materialization task derivation tests passed: 66 deterministic Atlas-seed tasks.')
