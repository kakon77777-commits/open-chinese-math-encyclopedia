...
assert.equal(atlas.entries.filter(x => x.maturity === 'canonical_mko').length, 9)
assert.equal((await getMaterializationQueue()).length, 71)
...
console.log('Core atlas tests passed: 80 nodes, 9 canonical MKOs, 71 queued seeds and negative graph/maturity cases.')
