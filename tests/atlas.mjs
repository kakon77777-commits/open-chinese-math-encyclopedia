...
assert.equal(atlas.entries.filter(x => x.maturity === 'canonical_mko').length, 14)
assert.equal((await getMaterializationQueue()).length, 66)
...
console.log('Core atlas tests passed: 80 nodes, 14 canonical MKOs, 66 queued seeds and negative graph/maturity cases.')
