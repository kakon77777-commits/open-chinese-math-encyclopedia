...
assert.equal(flattened.length, 64)
assert.equal(new Set(flattened.map(task => task.task_id)).size, 64)
...
console.log(`Materialization scheduler tests passed: ${realBatches.length} dependency-safe batches cover 64 tasks.`)
