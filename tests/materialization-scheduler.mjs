...
assert.equal(flattened.length, 66)
assert.equal(new Set(flattened.map(task => task.task_id)).size, 66)
...
console.log(`Materialization scheduler tests passed: ${realBatches.length} dependency-safe batches cover 66 tasks.`)
