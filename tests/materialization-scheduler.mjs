...
assert.equal(flattened.length, 65)
assert.equal(new Set(flattened.map(task => task.task_id)).size, 65)
...
console.log(`Materialization scheduler tests passed: ${realBatches.length} dependency-safe batches cover 65 tasks.`)
