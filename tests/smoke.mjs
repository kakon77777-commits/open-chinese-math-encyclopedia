...
assert.equal(theorem.version, '0.6.0')
...
assert.equal(theorem.verification.evidence_refs.length, 2)
assert.deepEqual(new Set(theorem.verification.evidence_refs.map(ref => ref.producer_id)), new Set(['ocme-python-suite', 'felra']))
...
assert.equal(compact.evidence_refs.length, 2)
...
console.log('Smoke tests passed: 3 MKO v0.3 objects, 4 evidence refs, active FELRA and 2 dependency edges.')
