...
export async function validateEscalationDecision(decision, {
  ...
  try {
    const expected = routeEscalation({ mechanicalReport, riskProfile, diversityProfile, disagreements })
    if (decision.route !== expected.route) errors.push(`${decision.decision_id}: route must equal ${expected.route}`)
    if (decision.blocking !== expected.blocking) errors.push(`${decision.decision_id}: blocking must equal ${expected.blocking}`)
    if (JSON.stringify(decision.reasons) !== JSON.stringify(expected.reasons)) {
      errors.push(`${decision.decision_id}: reasons must exactly match deterministic escalation policy`)
    }
    if (JSON.stringify(decision.open_disagreement_ids) !== JSON.stringify(expected.open_disagreement_ids)) {
      errors.push(`${decision.decision_id}: open_disagreement_ids must exactly match open disagreements`)
    }
  } catch (error) {
    errors.push(`${decision.decision_id}: escalation policy inputs invalid: ${error.message}`)
  }
