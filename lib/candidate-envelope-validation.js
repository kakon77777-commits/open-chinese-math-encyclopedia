...
import { findForbiddenProtocolKeys } from './protocol-authority-boundary.js'
...
export async function validateCandidateEnvelope(candidate, task, contract, { schema = null } = {}) {
  ...
  for (const forbiddenPath of findForbiddenProtocolKeys(candidate)) {
    errors.push(`${candidate.candidate_revision_id}: forbidden protocol authority field at ${forbiddenPath}`)
  }
