...
export function expandArchitectureProfile(seed, dimensionOrder = DIFFICULTY_DIMENSIONS) {
  ...
  const assessment = seed.assessment || {
    ...
    review_version: 1,
    review_rationale_zh: '此設定用於 v0.9 架構樣本，分離分類、難度、路徑與方法論。',
  }
  const reviewer = assessment.reviewer ? { reviewer: assessment.reviewer } : {}
  ...
  return {
    ...
    difficulty: {
      ...
      profiles: [{
        ...
        source: {
          ...
          reviewed: assessment.reviewed,
          ...reviewer,
          version: assessment.review_version,
        },
      }],
    },
    learning: {
    ...
    review: {
      ...
      ...reviewer,
      version: assessment.review_version,
      rationale_zh: assessment.review_rationale_zh,
    },
  }
}
