...
export async function validateCoreAtlasV02Candidate(atlas, {
  ...
  for (const entry of entries) {
    ...
    if (entry.maturity === 'canonical_mko') {
      ...
      if (objectIds.size && !objectIds.has(entry.canonical_mko_id)) errors.push(`${entry.id}: canonical MKO does not exist ${entry.canonical_mko_id}`)
    } else {
  ...
  const mismatches = canonicalDependencyMismatches(atlas, objects)
