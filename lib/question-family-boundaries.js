const NEW_FAMILY_SOURCES = Object.freeze({
  ...
  'qf-finite-set-union': 'mko-set-operations',
  'qf-finite-set-intersection': 'mko-set-operations',
  'qf-finite-set-difference': 'mko-set-operations',
  'qf-finite-relative-complement': 'mko-set-operations',
})
...
function canonicalFiniteSet(values) {
  const sorted = [...new Set(values)].sort((left, right) => left - right)
  return sorted.length ? `{${sorted.join(', ')}}` : '∅'
}

function validateFiniteSetInput(values, label, errors) {
  if (!Array.isArray(values) || !values.every(Number.isInteger)) {
    errors.push(`${label} must be an explicit finite integer set`)
    return []
  }
  if (new Set(values).size !== values.length) errors.push(`${label} may not contain duplicate set elements`)
  return values
}

function finiteSetOperationResult(family, parameters, errors) {
  const a = validateFiniteSetInput(parameters.A, 'A', errors)
  if (family === 'qf-finite-relative-complement') {
    const universe = validateFiniteSetInput(parameters.U, 'U', errors)
    if (!a.every(value => universe.includes(value))) errors.push('relative complement requires A to be a subset of U')
    const result = universe.filter(value => !a.includes(value))
    return { expected: canonicalFiniteSet(result), witness: result[0] ?? '∅' }
  }

  const b = validateFiniteSetInput(parameters.B, 'B', errors)
  if (family === 'qf-finite-set-union') {
    const result = [...new Set([...a, ...b])]
    const overlap = a.find(value => b.includes(value))
    return { expected: canonicalFiniteSet(result), witness: overlap ?? result[0] ?? '∅' }
  }
  if (family === 'qf-finite-set-intersection') {
    const result = a.filter(value => b.includes(value))
    return { expected: canonicalFiniteSet(result), witness: result[0] ?? '∅' }
  }
  const result = a.filter(value => !b.includes(value))
  return {
    expected: canonicalFiniteSet(result),
    witness: result[0] ?? '∅',
    reverse: canonicalFiniteSet(b.filter(value => !a.includes(value))),
  }
}

function substantiveParameters(family, parameters) {
  const { semantic_template: ignored, ...all } = parameters
  const normalizedFiniteInput = values => Array.isArray(values) ? [...values].sort((left, right) => left - right) : values
  if (family === 'qf-arithmetic-domain-boundary') {
    const { state: derivedState, ...substantive } = all
    return substantive
  }
  if (['qf-finite-set-union', 'qf-finite-set-intersection', 'qf-finite-set-difference'].includes(family)) {
    return { A: normalizedFiniteInput(parameters.A), B: normalizedFiniteInput(parameters.B) }
  }
  if (family === 'qf-finite-relative-complement') {
    return { U: normalizedFiniteInput(parameters.U), A: normalizedFiniteInput(parameters.A) }
  }
...
export function validateNewQuestionFamily(question) {
  ...
  if (family === 'qf-arithmetic-domain-boundary') {
  ...
  } else if (family === 'qf-basic-counting-principle') {
    ...
    result = finiteSetOperationResult(family, parameters, errors)
    if (!question.stem_zh.includes('元素只列一次') || !question.stem_zh.includes('依遞增排序')) errors.push('finite-set stem must declare canonical extensional output')
    if (family === 'qf-finite-set-union' && !question.stem_zh.includes('A∪B')) errors.push('union stem must name A∪B')
    if (family === 'qf-finite-set-intersection' && !question.stem_zh.includes('A∩B')) errors.push('intersection stem must name A∩B')
    if (family === 'qf-finite-set-difference') {
      if (!question.stem_zh.includes('方向性差集 A\\B')) errors.push('difference stem must preserve A\\B direction')
      if (!explanation.includes(`B\\A`) || !explanation.includes(result.reverse)) errors.push('difference explanation must name the independently recomputed reverse result')
    }
    if (family === 'qf-finite-relative-complement') {
      if (!question.stem_zh.includes('明示全集 U') || !question.stem_zh.includes('⊆U') || !question.stem_zh.includes('U\\A')) errors.push('relative-complement stem must explicitly bind U and A subset U')
      if (!explanation.includes('全集 U') || !explanation.includes('A⊆U')) errors.push('relative-complement explanation must preserve the universe witness')
    }
  }
