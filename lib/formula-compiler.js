...
const COMMAND_SYMBOLS = {
  ...
  epsilon: { type: 'symbol', name: 'epsilon', glyph: 'ε' },
  delta: { type: 'symbol', name: 'delta', glyph: 'δ' },
}

const RELATION_COMMANDS = new Set(['in', 'to', 'subseteq'])
const SET_OPERATION_COMMANDS = new Set(['cup', 'cap', 'setminus'])
...
class Parser {
  ...
  matchCommand(name) {
    ...
    if (token.type !== 'command' || token.value !== name) return false
    this.index += 1
    return true
  }
  expect(type) {
  ...
  expectCommand(name) {
    const token = this.current()
    if (token.type !== 'command' || token.value !== name) {
      throw new FormulaSyntaxError(`Expected \\${name}`, token.position, this.source)
    }
    this.index += 1
    return token
  }
  ...
  parseRelation() {
    const lhs = this.parseSetUnion()
    if (this.match(':')) {
      const domain = this.parseSetUnion()
      this.expectCommand('to')
      return { type: 'mapping', function: lhs, domain, codomain: this.parseSetUnion() }
    }
    if (this.matchCommand('in')) return { type: 'membership', element: lhs, set: this.parseSetUnion() }
    if (this.matchCommand('subseteq')) return { type: 'set_subset', subset: lhs, superset: this.parseSetUnion() }
    if (this.matchCommand('to')) return { type: 'tends_to', expression: lhs, target: this.parseSetUnion() }
    if (this.match('=')) return { type: 'equation', lhs, rhs: this.parseSetUnion() }
    return lhs
  }

  parseSetUnion() {
    let node = this.parseSetIntersection()
    while (this.matchCommand('cup')) {
      node = { type: 'set_union', left: node, right: this.parseSetIntersection() }
    }
    return node
  }
  ...
  parseSetIntersection() {
    ...
    while (true) {
      if (this.matchCommand('cap')) {
        node = { type: 'set_intersection', left: node, right: this.parseAdditive() }
      } else if (this.matchCommand('setminus')) {
        node = { type: 'set_difference', left: node, right: this.parseAdditive() }
      } else break
    }
    return node
  }
  ...
  parsePostfix() {
    ...
    while (true) {
      if (this.match('(')) {
        const args = []
        if (!this.match(')')) {
          do { args.push(this.parseRelation()) } while (this.match(','))
          this.expect(')')
        }
  ...
  parsePrimary() {
    ...
    if (this.match('command')) {
      if (RELATION_COMMANDS.has(token.value)) {
        throw new FormulaSyntaxError(`Relation command \\${token.value} requires a left operand`, token.position, this.source)
      }
      if (SET_OPERATION_COMMANDS.has(token.value)) {
        throw new FormulaSyntaxError(`Set operation command \\${token.value} requires a left operand`, token.position, this.source)
      }
      if (token.value === 'frac') {
        return {
          type: 'fraction',
...
function renderNode(node) {
  switch (node.type) {
    ...
    case 'membership': return `<mrow>${renderNode(node.element)}<mo>∈</mo>${renderNode(node.set)}</mrow>`
    case 'set_subset': return `<mrow>${renderNode(node.subset)}<mo>⊆</mo>${renderNode(node.superset)}</mrow>`
    case 'set_union': return `<mrow>${renderNode(node.left)}<mo>∪</mo>${renderNode(node.right)}</mrow>`
    case 'set_intersection': return `<mrow>${renderNode(node.left)}<mo>∩</mo>${renderNode(node.right)}</mrow>`
    case 'set_difference': return `<mrow>${renderNode(node.left)}<mo>∖</mo>${renderNode(node.right)}</mrow>`
    case 'mapping': return `<mrow>${renderNode(node.function)}<mo>:</mo>${renderNode(node.domain)}<mo>→</mo>${renderNode(node.codomain)}</mrow>`
    case 'tends_to': return `<mrow>${renderNode(node.expression)}<mo>→</mo>${renderNode(node.target)}</mrow>`
...
export function compileFormula(tex, { compilerVersion = '0.4.0' } = {}) {
