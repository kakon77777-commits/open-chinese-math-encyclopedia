...
class Parser {
  ...
  parseRelation() {
    ...
    if (this.match(':')) {
      ...
      return { type: 'mapping', function: lhs, domain, codomain: this.parseAdditive() }
    }
    if (this.matchCommand('in')) return { type: 'membership', element: lhs, set: this.parseAdditive() }
    if (this.matchCommand('to')) return { type: 'tends_to', expression: lhs, target: this.parseAdditive() }
