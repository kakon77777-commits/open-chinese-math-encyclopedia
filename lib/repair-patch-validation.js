...
import { findForbiddenProtocolKeys, isForbiddenProtocolKey } from './protocol-authority-boundary.js'
...
export function decodePointer(pathValue) {
  ...
  if (segments.some(segment => FORBIDDEN_SEGMENTS.has(segment) || isForbiddenProtocolKey(segment))) return null
  return segments
}
...
export async function validateRepairPatch(patch, { task, candidate, ledger, schema = null } = {}) {
  ...
  for (const operation of patch.operations) {
    ...
    if (!segments || !ALLOWED_ROOTS.has(segments[0])) {
      errors.push(`${patch.patch_id}: operation path is outside allowed candidate roots or crosses authority boundary: ${operation.path}`)
      continue
    }
    ...
    if (Object.hasOwn(operation, 'value')) {
      for (const forbiddenPath of findForbiddenProtocolKeys(operation.value)) {
        errors.push(`${patch.patch_id}: operation value contains forbidden protocol authority field at ${forbiddenPath}`)
      }
    }
  }
